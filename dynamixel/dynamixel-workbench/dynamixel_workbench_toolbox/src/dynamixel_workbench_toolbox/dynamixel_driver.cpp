...
bool DynamixelDriver::syncRead(uint8_t index, uint8_t *id, uint8_t id_num, const char **log) // index=thingtoread, id*=id array, id_num=number of motors
{
    ErrorFromSDK sdk_error = {0, false, false, 0};

    syncReadHandler_[index].groupSyncRead->clearParam(); // investigate syncreadhandler
    for (int i = 0; i < id_num; i++)                     // for each id 1-7
    {
        sdk_error.dxl_addparam_result = syncReadHandler_[index].groupSyncRead->addParam(id[i]); // investigate
        if (sdk_error.dxl_addparam_result != true)
        {
            if (log != NULL)
                *log = "groupSyncWrite addparam failed";
...
// index: syncReadHandler index, *id:1-7, id_num:7, *data:array to store data
